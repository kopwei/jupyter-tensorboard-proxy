...
def setup_tensorboardproxy():
    ...
    def _get_tensorboardproxy_command(port):
        executable = shutil.which('tensorboardproxy')
        if not executable:
            raise FileNotFoundError('Can not find tensorboardproxy executable in $PATH')
        # Create theia working directory
        home_dir = os.environ.get('HOME') or '/home/jovyan'
        working_dir = f'{home_dir}/tensorboardproxy'
        if not os.path.exists(working_dir):
            os.makedirs(working_dir)
            logger.info("Created directory %s" % working_dir)
        else:    
            logger.info("Directory %s already exists" % working_dir)
        return ['tensorboardproxy']
    
    return {
        'command': '_get_tensorboardproxy_command',
        'timeout': 20,
